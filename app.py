...
# 3. Header
# ======================================
st.title("⚡ Advanced Power Transmission Analysis Pro")
...
# ======================================
# Total Impedance and Admittance Calculation
if method == "Enter Z,Y,length (Per km)":
    ...
    # Exact Propagation Constant Calculations
...
# ─────────────────────────────────────────────────────────
# TAB 1 — High Fidelity Power Circle Diagrams Workspace
# ─────────────────────────────────────────────────────────
with tab1:
    st.markdown("### 🔍 Advanced Graphical Network Trajectory Analysis")
    
    # 1. Interactive Overlaid Plotly Control Center
    st.subheader("Interactive Combined Power Circle Diagram (Plotly Engine)")
    ...
    add_plotly_circle(fig_plotly, nr_x, nr_y, radius_r, "Receiving Circle Trajectory", "purple")
    add_plotly_circle(fig_plotly, ns_x, ns_y, radius_s, "Sending Circle Trajectory", "orange")
    ...
    fig_plotly.add_trace(go.Scatter(x=[0, Pr_mw], y=[0, qr_val], mode='lines+markers', name="Sr Complex Vector (Load)", line=dict(color='red', width=3.5)))
    ...
    fig_plotly.add_trace(go.Scatter(x=[0, ps_mw], y=[0, qs_plot], mode='lines+markers', name="Ss Complex Vector (Source)", line=dict(color='darkgreen', width=3.5)))
    ...
    fig_plotly.add_trace(go.Scatter(x=[nr_x, ns_x], y=[nr_y, ns_y], mode='markers+text', 
                                    ...
                                    marker=dict(color='yellow', size=10), name="Geometric Coordinates Centers"))

    fig_plotly.update_layout(
        template="plotly_dark", width=1000, height=750,
        xaxis=dict(title="Active Power Performance P (MW)", gridcolor='gray', zerolinecolor='white'),
        yaxis=dict(title="Reactive Power Tracking Q (MVAR)", gridcolor='gray', zerolinecolor='white', scaleanchor="x", scaleratio=1),
        hovermode="closest"
    )
    ...
    # 2. Complete High-Resolution Matplotlib Structural Visualizations
    st.subheader("Matplotlib Detailed Graphic Engine Plots")
    ...
    # Precalculating Arrays for circles mapping
    circle_theta = np.linspace(0, 2 * np.pi, 500)
    
    if p_choice == "Isolated Receiving-End Chart":
        fig_m, ax_m = plt.subplots(figsize=(8, 8))
        style_dark_plot(ax_m, fig_m, "Receiving-end Analytical Power Circle Diagram", "Active Power P (MW)", "Reactive Power Q (MVAR)")
        ax_m.plot(nr_x + radius_r * np.cos(circle_theta), nr_y + radius_r * np.sin(circle_theta), color='purple', linestyle='--', linewidth=1.5, label='Power Circle Domain')
        ...
        fig_m, ax_m = plt.subplots(figsize=(8, 8))
        style_dark_plot(ax_m, fig_m, "Sending-end Analytical Power Circle Diagram", "Sending Power Ps (MW)", "Sending Reactive Power Qs (MVAR)")
        ax_m.plot(ns_x + radius_s * np.cos(circle_theta), ns_y + radius_s * np.sin(circle_theta), color='orange', linestyle='--', linewidth=1.5, label='Sending Circle Trajectory')
        ...
        style_dark_plot(ax_m, fig_m, "Combined Multi-Machine Profile Power Overlay Plot", "Active Energy Load Vector P (MW)", "Reactive Energy Flow Q (MVAR)")
        ...
        ax_m.plot([0, ps_mw], [0, qs_plot], color='darkgreen', linewidth=3, label='$S_s$ (Sending Flux)')
        ...
        ax_m.axhline(Qmax, ls=':', color='#e14eca', alpha=0.7, label=f'Qmax Constraint limit ({Qmax} MVAR)')
        ax_m.axhline(Qmin, ls=':', color='#e14eca', alpha=0.7, label=f'Qmin Constraint limit ({Qmin} MVAR)')
...
# ─────────────────────────────────────────────────────────
with tab2:
    st.markdown("#### 📈 Voltage Stability Margin Assessment (PV Curve Analysis)")
    if Pmax_calc > 0:
        ...
        style_dark_plot(ax_pv, fig_pv, "Voltage Profile Tracking Curve", "Load Power P (MW)", "Terminal Required Sending Voltage |Vs| L-L (kV)")
        ax_pv.plot(P_range, V_list, color='#e14eca', lw=2.5)
        ax_pv.axvline(Pmax_calc, ls='--', color='#ff8d72', label=f"Critical Power Boundary Point (Pmax = {Pmax_calc:.1f} MW)")
        ax_pv.scatter(Pr_mw, vs_ll, color='#00d2ff', s=100, zorder=5, label="Active Operating Matrix Point")
...
# ─────────────────────────────────────────────────────────
with tab3:
    st.markdown("#### ⚡ Distributed Complex Voltage Wave Regulation Space")
    if method == "Enter Z,Y,length (Per km)" and line_length > 0:
        ...
        style_dark_plot(ax_prof, fig_prof, "Distributed Line Voltage Transient Drop Space", "Distance From Receiving Grid Bus Terminal (km)", "Line-to-Line Voltage Magnitude (kV)")
        ax_prof.plot(distance_line, V_profile_array, color='#00d2ff', lw=2.5)
        ax_prof.invert_xaxis() # Coordinates origin aligned from Receiving Node system
        st.pyplot(fig_prof)
    else:
        st.info("Voltage Distributed Hyperbolic Profiles tracking requires active direct Per-KM parameters modeling selection method.")

# ─────────────────────────────────────────────────────────
# TAB 4 — Mathematical Verification Summary Reports 
# ─────────────────────────────────────────────────────────
with tab4:
    st.markdown("#### 📋 Detailed Internal State Phasor Matrices")
    cs1, cs2 = st.columns(2)
    with cs1:
        st.markdown("**Node Electrical Complex Coordinates:**")
        st.code(
            f"Vr (Phase Tensor Base) = {polar_str(vr_ph, 'V')}\n"
            f"Ir (Line Vector Angle) = {polar_str(ir_vec, 'A')}\n"
            f"Vs (Phase Matrix Vector) = {polar_str(vs_vec, 'V')}\n"
            f"Is (Line Return Current) = {polar_str(is_vec, 'A')}\n"
            f"Vs (Line-To-Line Output) = {vs_ll:.4f} kV\n"
            f"Vr (Line-To-Line Target) = {vr_kv:.4f} kV"
        )
        st.markdown("**Transmission Grid Efficiency Matrices:**")
        st.code(
            f"Transmission Angle (δ) = {vs_angle:.4f}°\n"
            f"Voltage Regulation Factor= {v_reg:.4f} %\n"
            f"Operational Efficiency  = {efficiency:.4f} %\n"
            f"Active Losses (ΔP)      = {loss:.4f} MW"
        )
    with cs2:
        st.markdown("**ABCD Transmission Two-Port Constants Network Verification:**")
        st.code(
            f"A = D Parameters Vector = {polar_str(A)}\n"
            f"B Impedance Matrix Term = {polar_str(B, 'Ω')}\n"
            f"C Admittance Matrix Term = {polar_str(C, 'S')}"
        )
        st.markdown("**Power Circle Geometry Analytical Center Matrix Data:**")
        st.code(
            f"Center Coordinates n_r  = ({nr_x:.3f} MW, {nr_y:.3f} MVAR)\n"
            f"Center Coordinates n_s  = ({ns_x:.3f} MW, {ns_y:.3f} MVAR)\n"
            f"Calculated System Radius= {radius_r:.3f} MVA\n"
            f"Maximum System Power Capability Boundary (Pmax) = {Pmax_calc:.4f} MW\n"
            f"Load Level Status Metric= {loading_pc:.1f} %"
        )
...
# ─────────────────────────────────────────────────────────
with tab5:
    st.markdown("#### 💾 Historic Network Parameters Performance Summary Log")
    if st.session_state.history:
        headers_col = st.columns([1.5, 1, 1, 1, 1, 1, 1.5, 1])
        for slot, label in zip(headers_col, ["Mode","Pr (MW)","PF r","Ps (MW)","Qs (MVAR)","Loading %","Stability Check Status","Action"]):
