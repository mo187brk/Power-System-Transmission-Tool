...
# ─────────────────────────────────────────────────────────
# TAB 1 — Power Circle Diagram  (LECTURE-CORRECT)
# ─────────────────────────────────────────────────────────
with tab1:
    ...
    with col_plot:
        ...
        if run_anim:
            ...
            ax_a.plot(Pr_circle, Qr_circle, '#00d2ff', lw=2, label='RPCD (Receiving)', alpha=0.7)
            ax_a.plot(Ps_circle, Qs_circle, '#ff8d72', lw=2, label='SPCD (Sending)',   alpha=0.7)
            ...
            ax_a.scatter(Pmax_calc, 0, color='#e14eca', s=130, zorder=6, marker='^')
            ax_a.text(Pmax_calc + 2, 2, f'Pmax\n{Pmax_calc:.0f} MW', color='#e14eca', fontsize=11)
            ...
            # ── Circles ──────────────────────────────────────────
            ax.plot(Pr_circle, Qr_circle, color='#00d2ff', lw=2.2,
                    label='RPCD (Receiving)', zorder=3)
            ax.plot(Ps_circle, Qs_circle, color='#ff8d72', lw=2.2,
                    label='SPCD (Sending)', zorder=3)

            # ── Qmax / Qmin ───────────────────────────────────────
            ax.axhline(Qmax, ls='--', color='#e14eca', alpha=0.6,
                       label=f"Qmax = {Qmax:.0f}")
            ax.axhline(Qmin, ls='--', color='#e14eca', alpha=0.6,
                       label=f"Qmin = {Qmin:.0f}")

            # ── Centre markers ────────────────────────────────────
            ax.scatter(n_rx, n_ry, color='#00d2ff', s=70, marker='x',
                       linewidths=2, zorder=6)
            ax.scatter(n_sx, n_sy, color='#ff8d72', s=70, marker='x',
                       linewidths=2, zorder=6)
            ax.text(n_rx + R_circle * 0.03, n_ry + R_circle * 0.04,
                    'n_r', color='#00d2ff', fontsize=10)
            ax.text(n_sx + R_circle * 0.03, n_sy - R_circle * 0.07,
                    'n_s', color='#ff8d72', fontsize=10)

            # ── Radius vectors (dashed): centre → operating point ─
            ...
            # ── Operating points ──────────────────────────────────
            ax.scatter(Pr, Qr, color='#00d2ff', s=120,
                       edgecolors='white', zorder=7)
            ax.scatter(Ps, Qs, color='#ff8d72', s=120,
                       edgecolors='white', zorder=7)
            ax.text(Pr + R_circle * 0.03, Qr + R_circle * 0.04,
                    'Sr', fontsize=13, color='#00d2ff', fontweight='bold')
            ax.text(Ps + R_circle * 0.03, Qs - R_circle * 0.06,
                    'Ss', fontsize=13, color='#ff8d72', fontweight='bold')

            # ── Power Transfer line Sr → Ss ───────────────────────
            ax.plot([Pr, Ps], [Qr, Qs], color='#5cb85c', lw=2,
                    ls='--', label='Power Transfer Sr→Ss', zorder=4)

            # ── Pmax marker ───────────────────────────────────────
            ax.scatter(Pmax_calc, 0, color='#e14eca', s=150,
                       marker='^', zorder=8)
            ax.text(Pmax_calc + R_circle * 0.03, R_circle * 0.05,
                    f'Pmax = {Pmax_calc:.0f} MW',
                    color='#e14eca', fontsize=11, fontweight='bold')

            # ── φr arc ────────────────────────────────────────────
            phi_deg_plot = np.degrees(np.arctan2(Qr, Pr))
            arc_r  = R_circle * 0.18
            arc_th = np.linspace(0, np.radians(phi_deg_plot), 40)
            ax.plot(arc_r * np.cos(arc_th), arc_r * np.sin(arc_th),
                    color='#00d2ff', lw=1, ls=':')
            ax.text(arc_r * 1.1, arc_r * 0.25,
                    f'φr = {abs(phi_deg_plot):.1f}°',
                    color='#00d2ff', fontsize=10)

            # ── δ label ───────────────────────────────────────────
            ax.text(R_circle * 0.04, -R_circle * 0.08,
                    f'δ = {delta:.1f}°', color='white', fontsize=11,
                    bbox=dict(facecolor='#1e2233', alpha=0.85,
                              edgecolor='#444a5e', boxstyle='round,pad=0.3'))

            # ── β−α annotation ────────────────────────────────────
            ax.text(n_rx + R_circle * 0.04, n_ry - R_circle * 0.10,
                    f'β−α = {beta - alpha:.1f}°',
                    color='#aaaaaa', fontsize=9)

            # ── Axes ──────────────────────────────────────────────
            ax.axhline(0, color='#626b82', lw=1.2)
            ax.axvline(0, color='#626b82', lw=1.2)

            # ── Plot window ───────────────────────────────────────
            all_x  = [Pr, Ps, n_rx + R_circle, n_rx - R_circle,
                       n_sx + R_circle, n_sx - R_circle, 0, Pmax_calc]
            all_y  = [Qr, Qs, n_ry + R_circle, n_ry - R_circle,
                       n_sy + R_circle, n_sy - R_circle, 0]
            x_min, x_max = min(all_x), max(all_x)
            y_min, y_max = min(all_y), max(all_y)
            pad_x = (x_max - x_min) * 0.15
            pad_y = (y_max - y_min) * 0.15
            ax.set_xlim(x_min - pad_x, x_max + pad_x * 1.5)
    ...
    with col_info:
        ...
        st.info(f"**R circle:** {R_circle:.2f} MVA")
