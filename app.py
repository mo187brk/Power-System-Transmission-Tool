...
if run_anim:
    st.subheader("Operating Point Movement")
    load_values = np.linspace(0, Pmax_calc*1.2, 40)
    fig_anim, ax_anim = plt.subplots(figsize=(7,7))
    ax_anim.plot(Pr_circle, Qr_circle, 'b', label='Rotor')
    ax_anim.plot(Ps_circle, Qs_circle, 'r', label='Stator')
    for P_test in load_values:
        Q_test = P_test * np.tan(phi) if PF != 0 else 0
        ax_anim.scatter(P_test, Q_test, color='cyan', s=10)
    ax_anim.set_xlim(-window, window)
    ax_anim.set_ylim(-window, window)
    ax_anim.set_aspect('equal')
    ax_anim.grid()
    ax_anim.set_title("Stability Path")
    ax_anim.legend()
    st.pyplot(fig_anim)
